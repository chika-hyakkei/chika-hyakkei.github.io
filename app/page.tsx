...
function enemyFor(e:DungeonEnemy,floor:number):BattleEnemy{const scale=floor+(e.boss?9:e.kind*.35);const name=e.boss?BOSSES[e.kind%BOSSES.length]:ENEMIES[e.kind%ENEMIES.length];const hp=Math.floor((e.boss?55:13)+scale*(e.boss?8.5:3.7));return{...e,name,hp,maxHp:hp,atk:Math.floor((e.boss?7:4)+scale*(e.boss?2.15:1.45)),def:Math.floor(scale/(e.boss?2.5:3.2)),gold:Math.floor((e.boss?50:7)+scale*3.5),exp:Math.floor((e.boss?32:8)+scale*3.4)};}
...
export default function Home(){
  ...
  const [name,setName]=useState(""); const [job,setJob]=useState<JobKey>("warrior"); const [ready,setReady]=useState(false); const [muted,setMuted]=useState(false); const [pendingPurchase,setPendingPurchase]=useState<Gear|"potion"|null>(null); const audio=useRef<AudioContext|null>(null);
  ...
  useEffect(()=>{if(!run||muted)return;const explore=[110,0,165,196,147,0,165,131],battle=[110,147,110,165,123,165,147,196],shop=[220,277,330,277,247,330,277,220];const notes=run.phase==="battle"?battle:run.phase==="shop"?shop:explore;let step=0;const id=window.setInterval(()=>{const note=notes[step++%notes.length];if(note)beep(note,.13,.0065,step%4===0?"triangle":"square");},235);return()=>window.clearInterval(id);},[Boolean(run),run?.phase,muted,beep]);
  ...
  const win=(g:Run,b:BattleEnemy)=>{chime();const exp=g.exp+b.exp;let level=g.level;let remain=exp;let hp=g.hp,mp=g.mp;while(remain>=level*12){remain-=level*12;level++;hp+=5;mp+=2;}const bosses=g.bosses+(b.boss?1:0);const next={...g,phase:(b.boss?"bossChoice":"explore") as Phase,battle:null,enemies:g.enemies.filter(e=>e.id!==b.id),gold:g.gold+b.gold,totalGold:g.totalGold+b.gold,kills:g.kills+1,bosses,level,exp:remain,hp,mp,message:`${b.name}を倒した！ ${b.gold}Gを得た。${level>g.level?` レベル${level}へ！`:""}`};return next;};
  const battleAction=(mode:"attack"|"skill1"|"skill2"|"potion"|"flee")=>{if(mode==="attack"){beep(240,.05,.04,"sawtooth");setTimeout(()=>beep(110,.08,.03),45);}else if(mode==="skill1"||mode==="skill2"){beep(520,.07,.035);setTimeout(()=>beep(780,.12,.025),60);}else if(mode==="potion")chime();else beep(150,.12,.025);setRun(g=>{if(!g||g.phase!=="battle"||!g.battle)return g;let b={...g.battle},next={...g},message="";const st=stats(g);let [r,seed]=nextRandom(g.seed);next.seed=seed;if(mode==="potion"){if(g.potions<=0)return{...g,message:"回復薬を持っていない。"};const heal=Math.min(18+g.level*2,st.maxHp-g.hp);next.potions--;next.hp+=heal;return enemyRetaliates(next,b,`回復薬でHPを${heal}回復した。`);}if(mode==="flee"){if(b.boss)return{...g,message:"階層主からは逃げられない！"};if(r<.65+(g.job==="thief"?.2:0))return{...g,phase:"explore",battle:null,message:"闇へ身を隠し、逃げ切った。"};return enemyRetaliates(next,b,"逃げ道を塞がれた！");}
        ...
        <div className={`map biome-${Math.floor((run.floor-1)/10)%4}`} role="grid" aria-label={`地下${run.floor}階の地図`}>
          {run.map.flatMap((row,y)=>row.map((tile,x)=>{
            ...
            return <div key={`${x}-${y}`} className={`tile t-${tileName} ${visible?"seen":"hidden"} omen-${omen%4}`} role="gridcell">
              {visible&&(player?<span className="pixel-player" aria-label="探索者"><i/><i/><i/><i/><i/></span>:enemy?<span className={`map-monster species-${enemy.kind%10} ${enemy.boss?"map-boss":""}`} aria-label={enemy.boss?"階層主":"魔物"}><i/><i/><i/></span>:tile===">"?<span className="stairs-mark">▼</span>:tile==="$"&&!opened?<span className="chest-mark">▰</span>:tile==="S"?<span className="merchant-mark">商</span>:tile==="."&&omen===0?<span className="bones" aria-hidden="true">†</span>:tile==="."&&omen===7?<span className="fungus" aria-hidden="true">♠</span>:"")}
            </div>;
          }))}
        </div>
