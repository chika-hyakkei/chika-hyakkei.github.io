  ...
  return <main className="shell">
    ...
    {!run?<section className="start-panel">
      ...
      <section className="game-layout"><div className="game-card"><div className="floor-head"><span>地下 <b>{run.floor}</b> 階</span><span>{JOBS[run.job].name}・{run.name}</span><span>得点 {score(run).toLocaleString()}</span></div>
        <div className="map" role="grid" aria-label={`地下${run.floor}階の地図`}>
          {run.map.flatMap((row,y)=>row.map((tile,x)=>{
            ...
            return <div key={`${x}-${y}`} className={`tile t-${tileName} ${visible?"seen":"hidden"}`} role="gridcell">
              {visible&&(player?<span className="player">◆</span>:enemy?<span className={enemy.boss?"boss":"foe"}>{enemy.boss?"王":"魔"}</span>:tile===">"?"▼":tile==="$"&&!opened?"箱":tile==="S"?"商":"")}
            </div>;
          }))}
        </div>
      ...
      {run.phase==="battle"&&run.battle&&<div className="overlay"><div className="modal battle"><span className="modal-kicker">ENCOUNTER</span><div className={`monster ${run.battle.boss?"king":""}`}><i/><i/><i/><i/><b>{run.battle.boss?"王":"魔"}</b></div><h2>{run.battle.name}</h2><div className="enemy-hp"><i style={{width:`${run.battle.hp/run.battle.maxHp*100}%`}}/></div><div className="battle-actions"><button onClick={()=>battleAction("attack")}>たたかう</button><button onClick={()=>battleAction("skill1")}>{JOBS[run.job].skills[0]}</button><button onClick={()=>battleAction("skill2")}>{JOBS[run.job].skills[1]}</button><button onClick={()=>battleAction("potion")}>回復薬 ×{run.potions}</button><button onClick={()=>battleAction("flee")}>にげる</button></div></div></div>}
